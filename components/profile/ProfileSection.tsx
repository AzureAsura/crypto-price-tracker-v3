...
const ProfileSection = async ({ userId }: { userId: string }) => {
    ...
    <div className="px-4 md:px-0 md:w-[95vw] min-h-screen mx-auto pt-24 lg:flex-row flex-col flex gap-8">
      ...
      <div className="flex-1 flex flex-col gap-5">
        {chatData.map((item: any) => (
          <div key={item.id} className="group flex gap-4 p-6 rounded-2xl card">
            ...
            <div className="flex flex-col gap-1 w-full">
              <div className="flex items-center gap-2">
                <span className="font-bold text-white tracking-tight">{data?.name}</span>
              ...
              <p className="text-[14px] text-white/80 leading-relaxed max-w-2xl">{item.content}</p>
